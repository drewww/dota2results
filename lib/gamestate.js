		...
		if(lobbyId in this.lobbies) {
			...
			if("scoreboard" in snapshot && !lobby.draft_over) {
				...
				try {
					...
					winston.info("TRYING TO UNPACK NAMES WITH PARALLEL!!");
					async.parallel(_.map(picks, _.bind(function(pick) {
						...
						winston.info("Done with name lookup process: " + JSON.stringify(results));
					});

					_.each(picks, _.bind(function(value, index) {
						// run through each pick, see if we can get a new player name
						// for this person. These requests can take a while unfortunately.
						// but we're going to try to run them serially. 
					}, this));
