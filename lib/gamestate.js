...
GameStateTracker.prototype = {
	...
	handleEvents: function(lobby, snapshot) {
		...
		if(!_.isUndefined(lobby.lastSnapshot.radiant) && !_.isUndefined(lobby.lastSnapshot.dire) && 
			(lobby.lastSnapshot.radiant.tower_state != snapshot.radiant.tower_state ||
			lobby.lastSnapshot.dire.tower_state != snapshot.radiant.tower_state)
			) {

			// for the sake of making this easy, lets just squish the two masks
			// together and leave the rest of the logic unchanged. 
			var combinedPreviousTowerState = lobby.lastSnapshot.radiant.tower_state + 
				(lobby.lastSnapshot.dire.tower_state << 11);
			var combinedNewTowerState = snapshot.radiant.tower_state + 
				(snapshot.dire.tower_state << 11);
