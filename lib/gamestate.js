...
GameStateTracker.prototype = {
	...
	finish: function() {
		...
		winston.info("tracked: " + lobbiesTracked.length + " (this tick " + this.lobbiesThisTick.length + ")");
		...
		if(lobbiesMissed.length > 0) {
			...
			_.each(lobbiesMissed, _.bind(function(lobbyId) {
				...
				fs.writeFileSync("games/lobby_" + finishedGameLobby + ".json", JSON.stringify(finishedGameLobby));
	...
	getLobbyByTeamAndLeague: function(teams, leagueId) {
		...
		_.find(this.lobbies, function(lobby) {
			winston.info("checking lobby, league_id: " + lobby.lastSnapshot.league_id);
			if(leagueId==lobby.lastSnapshot.league_id) {
				// now look at teams
				if(_.isUndefined(lobby.teams)) {
					winston.info("\tno teams");
					return false;
				}

				var radiantTeamId = lobby.teams.radiant.team_id;
				var direTeamId = lobby.teams.dire.team_id;
				winston.info("radiant: " + radiantTeamId + "; dire: " + direTeamId + " teams: " + JSON.stringify(teams));

				winston.info("Checking finished game: " + radiantTeamId + " vs " + direTeamId + " in league " + lobby.lastSnapshot.league_id);
				if(_.contains(teams, radiantTeamId) && _.contains(teams, radiantTeamId)) {
					winston.info("found!");
					return true;
				}
			}
			winston.info("No match found.");
			return false;
		});
	},
