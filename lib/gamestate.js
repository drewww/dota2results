...
GameStateTracker.prototype = {
	...
	processSnapshot: function(snapshot) {
		...
		if(lobbyId in this.lobbies) {
			...
			// see if this lobby has pick_ban information
			// I'm not yet sure if we see lobbies pre pick_ban information being available
			// it may be that any lobby that shows results should have it.
			winston.info("snapshot keys: " + Object.keys(snapshot));
			winston.info("scoreboard keys: " + Object.keys(snapshot.scoreboard));
			if("pick_ban" in snapshot && !lobby.draft_over) {
				lobby.draft_over = true;
				lobby.draft = snapshot.pick_ban;
				winston.info("DRAFT OVER FOR LOBBY " + lobbyId);
				winston.info(JSON.stringify(lobby.draft));
			}
