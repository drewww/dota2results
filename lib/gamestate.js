					...
					async.parallel(_.map(picks, _.bind(function(pick) {
						...
						return _.bind(function(callback) {
							if(pick.account_id in this.playerNames) {
								winston.info("Found player name in cache: " + this.playerNames[pick.account_id] + " for id " + pick.account_id);
								callback(null, [pick.account_id, this.playerNames[pick.account_id]]);
							} else {
								// go find it.
								winston.info("Requesting real player name for " + pick.account_id);
						...
						_.each(results, function(tuple) {
							...
							if(name.length > 0) {
								...
								if(account_id in picks) {
									...
									winston.info("replacing name with looked-up name: " + name);
								}
							}
						});
						...
						draft.generate(lobby, _.bind(function(filename) {
							...
							winston.info("Time to tweet this!");

							// only tweet if it's league 600, eg ti.
							if(lobby.league_id==600) {
								...
								_.each(["radiant", "dire"], function(team) {
									var teamId = lobby[team + "_team"].team_id;
									var teamName = lobby[team + "_team"].team_name;
