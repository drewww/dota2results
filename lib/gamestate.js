var winston = require('winston'),
	...
	twitter_update_with_media = require('./twitter_update_with_media.js'),
...
function GameStateTracker(redis, silent) {
	...
	// this is a little messy since it's duplicative of the main object
	// but whatever. 
...
GameStateTracker.prototype = {
			...
			if("scoreboard" in snapshot && !lobby.draft_over) {
				...
				try {
					...
					async.parallel(_.map(picks, _.bind(function(pick) {
						...
						if(lobby.lastSnapshot.league_id==600 && !this.isSilent) {
							draft.generate(lobby, _.bind(function(filename) {
								...
								// only tweet if it's league 600, eg ti.
	...
	handleEvents: function(lobby, snapshot) {
		...
		if((lobby.lastSnapshot.scoreboard) &&
			...
			var raxTypes = [
				{type:"radiant", lane: "top", kind: "melee", mask: Math.pow(2, 0)},
				{type:"radiant", lane: "top", kind: "ranged", mask: Math.pow(2, 1)},
				{type:"radiant", lane: "mid", kind: "melee", mask: Math.pow(2, 2)},
				{type:"radiant", lane: "mid", kind: "ranged", mask: Math.pow(2, 3)},
				{type:"radiant", lane: "bot", kind: "melee", mask: Math.pow(2, 4)},
				{type:"radiant", lane: "bot", kind: "ranged", mask: Math.pow(2, 5)},
				{type:"dire", lane: "top", kind: "melee", mask: Math.pow(2, 6)},
				{type:"dire", lane: "top", kind: "ranged", mask: Math.pow(2, 7)},
				{type:"dire", lane: "mid", kind: "melee", mask: Math.pow(2, 8)},
				{type:"dire", lane: "mid", kind: "ranged", mask: Math.pow(2, 9)},
				{type:"dire", lane: "bot", kind: "melee", mask: Math.pow(2, 10)},
				{type:"dire", lane: "bot", kind: "ranged", mask: Math.pow(2, 11)}
			];
