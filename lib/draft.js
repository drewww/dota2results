...
function generate(game, cb) {
	...
	_.each(["radiant", "dire"], function(team) {
		...
		if(team=="radiant") {
			...
			coords = {x:offset, y:200, heroY: 200-70};
		}
		...
		_.each(_.filter(game.picks, function(pick) { return pick.team==team; }),
			function(player, index) {
				...
				util.drawImageForHeroId(player.hero, 20, 0, ctx, 36);
				winston.info(JSON.stringify(player));
				var textWidth = ctx.measureText(player.name);
