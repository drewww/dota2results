...
function generate(game, matchMetadata, cb) {
	...
	if(game.goldHistory.length<(game.lastSnapshot.scoreboard.duration/180.0)) {
    	winston.error("Too few gold ticks: " + game.goldHistory.length + " (expecting at least " + endTime / 180.0 + " ticks for " + endTime + " seconds)");
	...
	_.each(game.goldHistory, function(snapshot, index) {
		...
		for(var segment=0; segment <= Math.floor(Math.abs(snapshot.diff)/10000.0); segment++) {
			...
			if(snapshot.diff > 0) {
				...
				winston.info(curX + "," + (segment*10000*goldScaling) + "," +  (snapshot.time*timeScaling-curX) + ", " + (localDiff*goldScaling));
				ctx.fillRect(curX, segment*10000*goldScaling+1, snapshot.time*timeScaling-curX+1, localDiff*goldScaling-1);
				// ctx.strokeRect(curX, segment*10000*goldScaling+1*segment, snapshot.time*timeScaling-curX, localDiff*goldScaling-1*segment);
			} else {
				// ctx.fillStyle = "#80b238";
				// ctx.strokeStyle = "#80b238";
				// ctx.fillRect(curX, 0, snapshot.time*timeScaling-curX, localDiff*goldScaling*-1);
				// ctx.strokeRect(curX, snapshot.diff*goldScaling, snapshot.time*timeScaling-curX, localDiff*goldScaling*-1);
			}
	...
	ctx.fillText("@dota2results", -25, 120);
