...
function generate(game, matchMetadata, cb) {
	...
	_.each(game.goldHistory, function(snapshot, index) {
		...
		for(var segment=0; segment <= Math.floor(Math.abs(snapshot.diff)/10000.0); segment++) {
			...
			if(snapshot.diff > 0) {
				...
				winston.info(curX + ", " + ((segment*10000*goldScaling*-1) - (localDiff*goldScaling*-1)) + ", " + (snapshot.time*timeScaling-curX) + ", " + (localDiff*goldScaling*-1));
    ...
    _.each(game.events, function(towerEvent) {
    	...
    	// ctx.fillText("T", x-TOWER_BOX_SIZE, flip*towerOffset + (8*(towerEvent.tier-1)*flip));
	...
	_.each(["radiant", "dire"], function(team) {
		...
		if(teamLogo) {
			ctx.drawImage(logoImage, 5, coords.y-18, 40, logoImage.width/40.0*logoImage.height);
		}
