...
function generate(game, matchMetadata) {
	...
	var exists = fs.existsSync("games/joint_" + game.lobby_id + ".json");
	if(!exists) {
		fs.writeFileSync("games/joint_" + game.lobby_id + ".json", JSON.stringify(out));
	}
	...
	_.each(["radiant", "dire"], function(team) {
		...
		var textOffset = 0;
		if(team=="radiant") {
			coords = {x:offset, y:22};
		} else {
			coords = {x:offset, y:200-5};
		}
	...
	_.each(game.goldHistory, function(snapshot, index) {
		...
		if(index == game.goldHistory.length-1) {
			...
			// if we're really close to the 60 minute mark, leae it off.
	...
	ctx.beginPath();
	ctx.strokeStyle = "#707070";
	ctx.moveTo(timeScaling*3600.00, -4);
	ctx.lineTo(timeScaling*3600.00, +4);
	ctx.stroke();
	ctx.closePath();

	ctx.font = "12px Helvetica";
	ctx.fillStyle = "#fff";
	ctx.fillText("60m", timeScaling*3600.00-22, -6);

	ctx.beginPath();
	ctx.moveTo(timeScaling*3600.00/2, -2);
	ctx.lineTo(timeScaling*3600.00/2, +2);
	ctx.stroke();
	ctx.closePath();
    ...
    _.each(game.events, function(towerEvent) {
    	...
    	ctx.fillStyle = "#fff";
    	ctx.fillRect(x-TOWER_BOX_SIZE/2, flip*towerOffset + (8*(towerEvent.tier-1)*flip), TOWER_BOX_SIZE, TOWER_BOX_SIZE);
    });
	...
	saveCanvas(canvas, game.lastSnapshot.lobby_id);
