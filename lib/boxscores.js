...
function generate(game, matchMetadata, cb) {
    ...
    _.each(game.events, function(objectiveEvent) {
    	var flip = 1;
    	if(objectiveEvent.team=="dire") {
    		flip = -1;
    	}

    	// we can have two kinds of events:
    	//	1. towers 
    	//	2. barracks
    	// figure out which we've got
	   	var x = objectiveEvent.time * timeScaling;
	   	winston.info("x mid: " + x);
	    var TOWER_BOX_SIZE = 5;

    	if(objectiveEvent.type=="tower") {
    		towerEvent = objectiveEvent;


	    	ctx.fillStyle = "#fff";
	    	ctx.fillRect(x-TOWER_BOX_SIZE, flip*towerOffset + (12*(towerEvent.tier-1)*flip), TOWER_BOX_SIZE-2, TOWER_BOX_SIZE+1);
    	} else if (objectiveEvent.type=="barracks") {
    		barracksEvent = objectiveEvent;

    		var yOffset = TOWER_BOX_SIZE+2;

    		if(barracksEvent.kind=="melee") {
    			yOffset = -4;
 	  		}

	    	ctx.fillStyle = GOLD_COLOR;

 	  		ctx.fillRect(x-TOWER_BOX_SIZE-1, flip*towerOffset + (12*(2)*flip + yOffset),
 	  				TOWER_BOX_SIZE, 3);
    		// im thinking design-wise that I want something on either side
    		// of the towers? there's a risk because owers sometimes happen in
    		// rapid succession

	    	// winston.info("x: " + (x+5*xFlip));
	    	// ctx.save();
	    	// ctx.translate(x+5*xFlip, flip*(towerOffset+(8*2)));

	    	// ctx.beginPath();
	    	// ctx.moveTo(0,0);
	    	// ctx.lineTo(4*xFlip, (TOWER_BOX_SIZE+1)/2.0);
	    	// ctx.lineTo(0, (TOWER_BOX_SIZE+1));
	    	// ctx.closePath();
	    	// ctx.fill();
	    	// ctx.restore();
    	}
	...
	try {
		util.saveCanvas(canvas, game.lastSnapshot.lobby_id, cb);		
	} catch (e) {
