...
function generate(game, matchMetadata, cb) {
	...
	_.each(["radiant", "dire"], function(team) {
		...
		winston.info("width: " + logoImage.width);
		winston.info("height: " + logoImage.height);

		// the logic here is:
		// width/height is the aspect ratio
		// we want to maintain the aspect ratio of image and lock the width at 40px
		// wide.
		winston.info("computed height: " + (logoImage.width/logoImage.height)*30);
