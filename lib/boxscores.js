...
function generate(game, matchMetadata) {
	...
	_.each(["radiant", "dire"], function(team) {
		...
		var team_id = game.teams[team].team_id;
		var coords = {};
		...
		if(seriesStatus && series_type in seriesStatus) {
			...
			var wins = seriesStatus.teams[game.teams[team].team_id];			
			...
			for(var i=0; i<gamesToWin; i++) {
				...
				ctx.strokeRect(seriesCoords.x, seriesCoord.y, 20, 10);
				ctx.fillRect(seriesCoords.x, seriesCoord.y, 20, 10);
