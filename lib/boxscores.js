...
function generate(game, matchMetadata, cb) {
	...
	if(game.goldHistory.length<(endTime/180.0)) {
    	winston.error("Declining to generate a boxscores image because this lobby has too few gold events");
    	winston.error("for " + game.lastSnapshot.radiant_team.team_name + " and " + game.lastSnapshot.dire_team.team_name);
    	winston.error("total gold ticks: " + game.goldHistory.length + " (expecting at least " + endTime / 180.0 + " ticks for " + endTime + " seconds)");
		return false;
	}
